...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        if isinstance(self.module._fsdp_wrapped_module, PeftModel):
            ...
            # 关键：PeftModel 本身一般没有 _checkpoint_conversion_mapping，要用 base_model.model
            base_model = self.module._fsdp_wrapped_module.base_model.model
            params = convert_weight_keys(params, base_model)
        else:
            params = self.module.state_dict()
            # 关键：用真实 HF 模型来做 key 回滚映射
            hf_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
            params = convert_weight_keys(params, hf_model)
